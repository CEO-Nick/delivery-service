...
public interface StoreMenuRepository extends JpaRepository<StoreMenuEntity, Long> {
    ...
    Optional<StoreMenuEntity> findFirstByIdAndStatusOrderByIdDesc(Long id, StoreMenuStatus menuStatus);

    // 특정 가게의 메뉴
    // select * from store_menu where store_id = ? and status = ? order by sequence desc
    List<StoreMenuEntity> findAllByStoreIdAndStatusOrderBySequenceDesc(Long storeId, StoreMenuStatus menuStatus);
}
